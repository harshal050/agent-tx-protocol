...
mod tests {
    ...
    fn local_backtrack_deepens_then_escalates_to_global_reset() {
        ...
        assert_eq!(
            ...
            RollbackDecision::LocalBacktrack {
                target_step: 4,
                depth: 1
            }
        );
        ...
        assert_eq!(
            ...
            RollbackDecision::GlobalReset {
                reason: EscalationReason::LocalDepthExhausted
            }
        );
    ...
    fn aborts_when_global_resets_exhausted() {
        ...
        assert!(matches!(
            c.decide(3, None, &graph),
            RollbackDecision::GlobalReset { .. }
        ));
        match c.decide(3, None, &graph) {
            RollbackDecision::Abort { reason } => {
                assert!(
                    reason.contains("global reset budget (1) exhausted"),
                    "{reason}"
                );
            }
    ...
    fn replay_budget_forces_global_reset_before_quadratic_replay() {
        ...
        assert_eq!(
            c.decide(7, None, &g),
            RollbackDecision::GlobalReset {
                reason: EscalationReason::ReplayBudgetExceeded
            }
        );
    }
