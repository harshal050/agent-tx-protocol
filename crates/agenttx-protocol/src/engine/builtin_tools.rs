...
impl Tool for KvPut {
    ...
    async fn execute(
        ...
        ctx.store().put(
            &format!("kv/{key}"),
            &serde_json::to_vec(&value).map_err(AgentTxError::from)?,
        )?;
...
mod tests {
    ...
    async fn kv_roundtrip() {
        ...
        run(
            ...
            &ctx,
            "kv.put",
            json!({ "key": "a", "value": { "n": 1 } }),
        )
        .await
        .unwrap();
        let got = run(&f, &ctx, "kv.get", json!({ "key": "a" }))
            .await
            .unwrap();
        assert_eq!(got["value"], json!({ "n": 1 }));
        let deleted = run(&f, &ctx, "kv.delete", json!({ "key": "a" }))
            .await
            .unwrap();
    ...
    async fn record_insert_enforces_primary_and_foreign_keys() {
        ...
        assert!(
            fk.contains("Key (user_id)=(101) is not present in table \"users\""),
            "{fk}"
        );
        ...
        let user = run(
            ...
            &ctx,
            "record.insert",
            json!({ "table": "users", "id": 101, "fields": { "name": "Ada" } }),
        )
        .await
        .unwrap();
        ...
        let null = failure(
            run(
                ...
                &ctx,
                "record.insert",
                json!({ "table": "orders", "id": "o2", "references": { "user_id": "users" } }),
            )
            .await,
        );
    ...
    async fn fs_write_registers_restorable_undo() {
        ...
        run(
            ...
            &ctx,
            "fs.write",
            json!({ "path": "notes/a.txt", "contents": "v1" }),
        )
        .await
        .unwrap();
        run(
            ...
            &ctx,
            "fs.write",
            json!({ "path": "notes/a.txt", "contents": "v2" }),
        )
        .await
        .unwrap();
        assert_eq!(std::fs::read_to_string(&file).unwrap(), "v2");
        let read = run(&f, &ctx, "fs.read", json!({ "path": "notes/a.txt" }))
            .await
            .unwrap();
    ...
    async fn effect_stage_defers_effects() {
        ...
        let out = run(
            ...
            &ctx,
            "effect.stage",
            json!({ "channel": "email", "payload": { "to": "a@b.c" }, "idempotency_key": "k1" }),
        )
        .await
        .unwrap();
