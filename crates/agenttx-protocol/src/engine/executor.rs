...
impl ToolContext {
    ...
    /// Drains the undo actions registered so far (used by the engine and by
    /// custom step runners such as the benchmark harness).
    pub fn take_undo(&self) -> Vec<Arc<dyn UndoAction>> {
        std::mem::take(&mut *self.undo.lock().unwrap_or_else(|p| p.into_inner()))
    }

    /// Drains the effects staged so far.
    pub fn take_staged(&self) -> Vec<StagedEffect> {
...
impl StepExecutor {
    ...
    pub async fn run(
        ...
        match tokio::time::timeout(self.timeout, &mut handle).await {
            ...
            Ok(Err(join)) => Err(ToolError::failed(format!(
                "tool '{name}' was cancelled: {join}"
            ))),
            Err(_) => {
                handle.abort();
                Err(ToolError::failed(format!(
...
fn resolve_string(
    ...
    for caps in REFERENCE.captures_iter(s) {
        ...
        let target_key = if field.is_empty() {
            source_key.clone()
        } else {
            field.to_owned()
        };
...
mod tests {
    ...
    fn future_or_unknown_steps_are_rejected() {
        for arg in ["${steps.2.id}", "${steps.1.id}", "${steps.99999999999.id}"] {
            ...
            assert!(
                failure.message.contains("has not completed"),
                "{arg}: {}",
                failure.message
            );
        }
    ...
    fn ctx() -> (tempfile::TempDir, Arc<ToolContext>) {
        ...
        (
            dir,
            Arc::new(ToolContext::new(store.scoped("tx").unwrap(), 1)),
        )
    }
    ...
    async fn executor_converts_timeouts_and_panics_into_failures() {
        ...
        let ok = exec
            .run(Arc::clone(&tool), Arc::clone(&ctx), json!({ "ms": 1 }))
            .await;
        assert_eq!(ok.unwrap(), json!({ "slept": true }));

        let timeout = exec
            .run(Arc::clone(&tool), Arc::clone(&ctx), json!({ "ms": 5_000 }))
            .await;
        assert!(
            matches!(timeout, Err(ToolError::Failed(m)) if m.contains("timed out after 50 ms"))
        );
    ...
    async fn context_collects_side_effect_declarations() {
        ...
        ctx.register_undo(Arc::new(crate::ledger::FnUndo::new("noop", || async {
            Ok(())
        })));
