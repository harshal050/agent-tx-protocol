...
pub static RULES: &[Rule] = &[
    ...
    Rule {
        ...
        render: |c| {
            ...
            format!(
                "Hint: Step {step} output has no '{key}'. Re-run step {step} so it returns '{key}', or fix the reference."
            )
        },
    },
    Rule {
    ...
    Rule {
        ...
        key_groups: &[],
        render: |_| {
            "Hint: Foreign key constraint failed. Ensure every referenced row exists before step execution.".into()
        },
    },
    Rule {
        name: "pg_unique",
    ...
    Rule {
        ...
        render: |c| {
            let key = get(c, &["key"]);
            format!(
                "Hint: Missing key '{key}' in input payload. Verify the upstream step output provides '{key}'."
            )
        },
    },
    Rule {
        ...
        render: |c| {
            let key = get(c, &["key"]);
            format!(
                "Hint: {}() requires argument '{key}'. Pass '{key}' in the tool arguments.",
                get(c, &["func"])
            )
        },
    },
    ...
    Rule {
        ...
        render: |c| match pick(c, &["key", "key2"]) {
            ...
            None => {
                "Hint: Null reference encountered. Validate that all required inputs are non-null."
                    .into()
            }
        },
    },
    ...
    Rule {
        ...
        render: |c| match pick(c, &["path", "path2", "path3"]) {
            Some(path) => format!(
                "Hint: File '{path}' does not exist. Create it in an earlier step or correct the path."
            ),
            None => "Hint: File does not exist. Create it in an earlier step or correct the path."
                .into(),
        },
    },
    Rule {
        ...
        render: |c| match pick(c, &["path", "path2"]) {
            Some(path) => format!(
                "Hint: Permission denied for '{path}'. Use a path inside the writable sandbox."
            ),
            None => {
                "Hint: Permission denied. Use a resource the agent is allowed to modify.".into()
            }
        },
    },
    // --- Network / HTTP ----------------------------------------------------
    Rule {
        ...
        key_groups: &[],
        render: |_| {
            "Hint: Rate limited by upstream API. Reduce request frequency or batch the calls."
                .into()
        },
    },
    Rule {
    ...
    Rule {
        ...
        key_groups: &[],
        render: |_| {
            "Hint: Upstream resource not found (HTTP 404). Verify the URL or resource identifier."
                .into()
        },
    },
    Rule {
    ...
    Rule {
        ...
        key_groups: &[],
        render: |_| {
            "Hint: Operation timed out. Reduce the work done in this step or split it into smaller steps.".into()
        },
    },
    Rule {
        ...
        key_groups: &[],
        render: |_| {
            "Hint: Dependency is unreachable. Verify the host/port or use an available service."
                .into()
        },
    },
    Rule {
        ...
        key_groups: &[],
        render: |_| {
            "Hint: Malformed JSON. Emit a single valid JSON object with double-quoted keys.".into()
        },
    },
];
