...
impl SagaLedger {
    ...
    pub fn recover(
        ...
        *ledger
            .entries
            .try_lock()
            .map_err(|_| AgentTxError::Compensation {
                action: "recover".into(),
...
pub trait EffectDispatcher: Send + Sync {
    async fn dispatch(&self, tx_id: &str, effect: &StagedEffect)
...
mod tests {
    ...
    async fn retries_then_reports_failures_without_stopping() {
        ...
        assert_eq!(
            report.failures,
            vec!["flaky: remote unavailable".to_string()]
        );
    ...
    async fn recovers_persisted_actions_through_registry() {
        ...
        let total = Arc::new(AtomicU32::new(0));
        {
            let ledger = SagaLedger::new("tx", Arc::clone(&store), 1);
            ledger
                .record(1, Arc::new(CounterUndo(Arc::clone(&total), 2)))
                .await
                .unwrap();
            ledger
                .record(2, Arc::new(CounterUndo(Arc::clone(&total), 5)))
                .await
                .unwrap();
            ledger
                .record(2, Arc::new(FnUndo::new("volatile", || async { Ok(()) })))
                .await
                .unwrap();
        }
        ...
        ledger
            .record(3, Arc::new(CounterUndo(Arc::clone(&total), 1)))
            .await
            .unwrap();
    ...
    async fn outbox_appends_json_lines() {
        ...
        outbox
            .dispatch("tx1", &effect(1, "email", "a"))
            .await
            .unwrap();
        outbox
            .dispatch("tx1", &effect(2, "webhook", "b"))
            .await
            .unwrap();
