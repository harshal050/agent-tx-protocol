...
/// Read options confining an iterator to `[lower, upper)`.
///
/// Every scan in this module is bounded: deleted keys (point tombstones) of
/// neighbouring transactions are never visited, so scans cost O(live keys in
/// range) no matter how many transactions have come and gone.
fn bounded(lower: &[u8], upper: Vec<u8>) -> ReadOptions {
...
impl RocksStore {
    ...
    pub fn open(path: impl AsRef<Path>, options: StoreOptions) -> Result<Self> {
        ...
        let version = match store
            .db
            .get_cf(store.cf(CF_SNAPSHOTS)?, keys::COMMIT_VERSION)?
        {
    ...
    pub fn get_output(&self, tx: &str, step_id: u32) -> Result<Option<Vec<u8>>> {
        Ok(self
            .db
            .get_cf(self.cf(CF_DEFAULT)?, keys::output(tx, step_id))?)
    }
    ...
    fn load_next_journal_seq(&self, tx: &str) -> Result<u64> {
        ...
        let mut iter = self.db.iterator_cf_opt(
            self.cf(CF_SNAPSHOTS)?,
            bounded(&prefix, upper),
            IteratorMode::End,
        );
        match iter.next().transpose()? {
            Some((key, _)) => keys::journal_seq_of(&key)
                .map(|seq| seq + 1)
    ...
    pub fn snapshot_meta(&self, id: &SnapshotId) -> Result<Option<SnapshotMeta>> {
        ...
        self.db
            .get_cf(
                self.cf(CF_SNAPSHOTS)?,
                keys::snapshot(&id.tx_id, id.step_id),
            )?
    ...
    pub fn restore_snapshot(&self, id: &SnapshotId) -> Result<RestoreStats> {
        ...
        for item in self
            .db
            .iterator_cf_opt(snaps, bounded(&lower, upper), IteratorMode::End)
        {
            let (key, value) = item?;
            debug_assert!(key.starts_with(&prefix) && key.as_ref() >= lower.as_slice());
        ...
        if let Some(next_step) = id.step_id.checked_add(1) {
            ...
            let snapshot_prefix = keys::snapshot_prefix(tx);
            let end = keys::prefix_end(&snapshot_prefix);
            for item in self
                .db
                .iterator_cf_opt(snaps, bounded(&later, end), IteratorMode::Start)
            {
                let (key, _) = item?;
                batch.delete_cf(snaps, key);
            }
    ...
    pub fn commit(&self, tx: &str) -> Result<usize> {
        ...
        let overlay_end = keys::prefix_end(&prefix);
        for item in view.iterator_cf_opt(data, bounded(&prefix, overlay_end), IteratorMode::Start) {
            ...
            if self.committed_version(key)? != base_version {
                return Err(AgentTxError::WriteConflict {
                    key: key.to_owned(),
                });
            }
    ...
    fn add_cleanup(&self, batch: &mut WriteBatch, tx: &str) -> Result<()> {
        ...
        self.delete_prefix(batch, CF_SAGA_LOGS, &keys::saga_prefix(tx))?;
        batch.delete_cf(self.cf(CF_SNAPSHOTS)?, keys::tx_record(tx));
        Ok(())
    }

    fn add_state_cleanup(&self, batch: &mut WriteBatch, tx: &str) -> Result<()> {
        self.delete_prefix(batch, CF_DEFAULT, &keys::tx_data_prefix(tx))?;
        self.delete_prefix(batch, CF_SNAPSHOTS, &keys::journal_prefix(tx))?;
        self.delete_prefix(batch, CF_SNAPSHOTS, &keys::snapshot_prefix(tx))?;
        Ok(())
    }

    /// Adds a point delete for every key under `prefix`.
    ///
    /// Point tombstones are used instead of range deletes: range tombstones
    /// in the memtable are re-fragmented on reads, so thousands of commits
    /// would make every later iterator progressively slower.
    fn delete_prefix(&self, batch: &mut WriteBatch, cf: &'static str, prefix: &[u8]) -> Result<()> {
        let handle = self.cf(cf)?;
        let end = keys::prefix_end(prefix);
        for item in self
            .db
            .iterator_cf_opt(handle, bounded(prefix, end), IteratorMode::Start)
        {
            let (key, _) = item?;
    ...
    fn scan_prefix(&self, cf: &'static str, prefix: &[u8]) -> Result<Vec<RawEntry>> {
        let end = keys::prefix_end(prefix);
        self.db
            .iterator_cf_opt(self.cf(cf)?, bounded(prefix, end), IteratorMode::Start)
...
mod tests {
    ...
    fn overlay_is_isolated_until_commit() {
        ...
        assert_eq!(
            store.get_committed("k").unwrap().as_deref(),
            Some(&b"v1"[..])
        );
    ...
    fn restore_reverts_to_exact_snapshot_state() {
        ...
        assert!(
            store
                .snapshot_meta(&SnapshotId::new("tx", 2))
                .unwrap()
                .is_none()
        );
    ...
    fn missing_snapshot_is_an_error() {
        let (_dir, store) = open();
        let err = store
            .restore_snapshot(&SnapshotId::new("tx", 9))
            .unwrap_err();
        assert!(matches!(
            err,
            AgentTxError::SnapshotNotFound { step_id: 9, .. }
        ));
    }
    ...
    fn first_committer_wins_on_write_conflict() {
        ...
        assert_eq!(
            store.get_committed("shared").unwrap().as_deref(),
            Some(&b"from-a"[..])
        );
    ...
    fn discard_and_commit_remove_all_tx_records() {
        ...
        assert!(
            store
                .snapshot_meta(&SnapshotId::new("tx", 0))
                .unwrap()
                .is_none()
        );
        assert_eq!(store.get("tx", "k").unwrap(), None);
