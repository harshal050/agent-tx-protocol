...
use crate::engine::{
    Engine, RollbackPolicy, RollbackStrategy, StepOutcome, StepRequest, StepStatus,
};
...
fn policy_from_proto(p: proto::RollbackPolicy, defaults: RollbackPolicy) -> RollbackPolicy {
    RollbackPolicy {
        ...
        replay_budget_factor: p
            .replay_budget_factor
            .unwrap_or(defaults.replay_budget_factor),
    }
}
...
impl AgentTxService for AgentTxGrpcService {
    async fn begin_transaction(
        ...
        let outcome = self
            .engine
            .begin(&req.agent_id, req.metadata, policy)
            .await?;
...
mod tests {
    ...
    fn enums_map_to_proto() {
        assert_eq!(
            strategy_to_proto(RollbackStrategy::DependencyJump),
            proto::RollbackStrategy::DependencyJump
        );
        assert_eq!(
            status_to_proto(StepStatus::RollbackTriggered),
            proto::StepStatus::RollbackTriggered
        );
    }
}
