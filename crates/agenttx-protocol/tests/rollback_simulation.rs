...
impl Tool for AlwaysFail {
    ...
    async fn execute(&self, _ctx: &ToolContext, args: Value) -> Result<Value, ToolError> {
        ...
        Err(ToolError::failed(
            args["error"].as_str().unwrap_or("boom").to_owned(),
        ))
    }
}
...
async fn begin(engine: &Engine) -> String {
    engine
        .begin("sim-agent", HashMap::new(), None)
        .await
        .unwrap()
        .tx_id
}
...
async fn local_backtracks_escalate_to_global_reset_then_abort() {
    ...
    let plan = [
        ...
        (
            "sim.fail",
            json!({ "error": "RuntimeError: widget exploded" }),
        ),
    ];
    ...
    assert!(
        reset
            .constraints
            .iter()
            .any(|c| c.contains("RuntimeError: widget exploded"))
    );
    ...
    assert!(matches!(
        h.engine.get(&tx).await,
        Err(AgentTxError::TransactionNotFound(_))
    ));
...
async fn dependency_jump_rewinds_state_compensates_saga_and_discards_effects() {
    ...
    assert_eq!(
        step(
            ...
            2,
            "fs.write",
            json!({ "path": "report.txt", "contents": "draft" })
        )
        .await
        .status,
        StepStatus::Success
    ...
    assert_eq!(
        step(
            ...
            3,
            "effect.stage",
            json!({ "channel": "email", "payload": { "to": "cfo@example.com" } })
        )
        .await
        .status,
        StepStatus::Success
    ...
    step(
        ...
        2,
        "fs.write",
        json!({ "path": "report.txt", "contents": "final" }),
    )
    .await;
    step(
        &h.engine,
    ...
    assert!(
        h.engine
            .store()
            .get_committed("records/invoices/inv-1")
            .unwrap()
            .is_some()
    );
...
async fn unresolved_reference_jumps_to_producer() {
    ...
    let failed = step(
        ...
        3,
        "kv.put",
        json!({ "key": "copy", "value": "${steps.1.user_id}" }),
    )
    .await;
    assert_eq!(failed.strategy, RollbackStrategy::DependencyJump);
    assert_eq!(failed.rollback_to_step, 1);
...
async fn global_reset_clears_context_and_keeps_constraints() {
    ...
    let reset = step(
        ...
        3,
        "sim.fail",
        json!({ "error": "HTTP 503 Service Unavailable" }),
    )
    .await;
    ...
    assert_eq!(
        view.constraints,
        vec![
            "Hint: Upstream service failed (HTTP 503). Retry later or use an alternative service."
                .to_string()
        ]
    );
...
async fn snapshot_restore_of_thousands_of_writes_is_sub_50ms() {
    ...
    assert!(
        store
            .snapshot_meta(&SnapshotId::new("bench", 1))
            .unwrap()
            .is_some()
    );
}
...
async fn crash_recovery_compensates_interrupted_transactions() {
    ...
    step(
        ...
        1,
        "fs.write",
        json!({ "path": "partial.txt", "contents": "half-done" }),
    )
    .await;
    step(
        &h.engine,
...
async fn concurrent_transactions_are_isolated_and_all_commit() {
            ...
            tokio::spawn(async move {
                ...
                for s in 1..=5u32 {
                    let out = step(
                        ...
                        s,
                        "kv.put",
                        json!({ "key": format!("t{i}-s{s}"), "value": i }),
                    )
                    .await;
                ...
                assert!(
                    engine
                        .store()
                        .get(&tx, &format!("kv/{other}"))
                        .unwrap()
                        .is_none()
                        || engine
                            .store()
                            .get_committed(&format!("kv/{other}"))
                            .unwrap()
                            .is_some()
                );
    ...
    for i in 0..16 {
        assert!(
            h.engine
                .store()
                .get_committed(&format!("kv/t{i}-s5"))
                .unwrap()
                .is_some()
        );
    }
...
async fn idle_transactions_are_reaped() {
    ...
    step(
        ...
        1,
        "fs.write",
        json!({ "path": "stale.txt", "contents": "x" }),
    )
    .await;
