...
impl TestServer {
    async fn start() -> Self {
        ...
        let store = Arc::new(RocksStore::open(db.path().join("rocks"), StoreOptions::default()).unwrap());
        ...
        let client = AgentTxServiceClient::connect(format!("http://{addr}")).await.unwrap();
    ...
    async fn step(&mut self, tx: &str, step_id: u32, tool: &str, args: Value) -> ExecuteStepResponse {
        self.client
...
async fn begin_execute_commit_publishes_state_and_dispatches_effects() {
    ...
    let put = srv.step(&tx, 1, "kv.put", json!({ "key": "greeting", "value": "hello" })).await;
    assert_eq!(put.status(), StepStatus::Success);
    assert_eq!(put.next_step_id, 2);
    assert_eq!(serde_json::from_str::<Value>(&put.output_json).unwrap()["value"], "hello");

    let staged = srv
        .step(&tx, 2, "effect.stage", json!({ "channel": "email", "payload": { "to": "ops@example.com" } }))
        .await;
    assert_eq!(staged.status(), StepStatus::Success);
    assert!(!srv.outbox.exists(), "effects must not be dispatched before commit");

    // Uncommitted state is invisible outside the transaction.
    assert!(srv.engine.store().get_committed("kv/greeting").unwrap().is_none());

    let view = srv
        .client
        .get_transaction(GetTransactionRequest { transaction_id: tx.clone() })
        .await
        .unwrap()
        .into_inner();
    ...
    let commit = srv
        .client
        .commit_transaction(CommitTransactionRequest { transaction_id: tx.clone() })
        .await
        .unwrap()
        .into_inner();
    ...
    let committed = srv.engine.store().get_committed("kv/greeting").unwrap().unwrap();
    assert_eq!(serde_json::from_slice::<Value>(&committed).unwrap(), json!("hello"));
...
async fn foreign_key_failure_triggers_dependency_jump() {
    ...
    srv.step(&tx, 1, "kv.put", json!({ "key": "selected_user", "value": "u-404" })).await;
    srv.step(&tx, 2, "fs.write", json!({ "path": "draft.txt", "contents": "order draft" })).await;
    ...
    assert!(srv.engine.store().get(&tx, "kv/selected_user").unwrap().is_none());
    assert!(!srv.sandbox.path().join("draft.txt").exists());

    // The agent follows the hint and retries from step 1.
    srv.step(&tx, 1, "record.insert", json!({ "table": "users", "id": "u-1" })).await;
    let ok = srv
        .step(
            &tx,
            2,
    ...
    srv.client
        ...
        .await
        .unwrap();
    assert!(srv.engine.store().get_committed("records/orders/o-1").unwrap().is_some());
    srv.stop().await;
}
...
async fn protocol_violations_map_to_grpc_status_codes() {
    ...
    let unknown = srv
        .client
        .commit_transaction(CommitTransactionRequest { transaction_id: "nope".into() })
        .await
        .unwrap_err();
    ...
    assert!(hallucinated.clean_hint.starts_with("Hint: Tool 'sql.run' does not exist. Use one of:"));
    srv.stop().await;
}
...
async fn manual_rollback_and_full_abort() {
    ...
    let tx = srv.begin().await;
    for step in 1..=3 {
        let r = srv.step(&tx, step, "kv.put", json!({ "key": format!("k{step}"), "value": step })).await;
...
async fn write_conflict_aborts_second_committer() {
    ...
    srv.step(&a, 1, "kv.put", json!({ "key": "inventory", "value": 9 })).await;
    srv.step(&b, 1, "kv.put", json!({ "key": "inventory", "value": 7 })).await;
    ...
    let conflict = srv
        .client
        .commit_transaction(CommitTransactionRequest { transaction_id: b.clone() })
        .await
        .unwrap_err();
    assert_eq!(conflict.code(), Code::Aborted);
    assert!(conflict.message().contains("inventory"));

    let value = srv.engine.store().get_committed("kv/inventory").unwrap().unwrap();
...
async fn health_service_reports_serving() {
    ...
    let response = health
        ...
        .unwrap()
        .into_inner();
    assert_eq!(response.status(), tonic_health::pb::health_check_response::ServingStatus::Serving);
    srv.stop().await;
}
