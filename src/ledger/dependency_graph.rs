...
mod tests {
    ...
    fn infers_implicit_dependency_by_key_and_value() {
        ...
        g.add_step(1, "record.insert", &json!({"table": "users", "id": "u-101"}), &[]);
    ...
    fn root_cause_follows_renames_to_earliest_producer() {
        ...
        g.add_step(2, "lookup", &json!({"user": 101}), &[explicit(1, "id", "user")]);
        g.record_output(2, &json!({"owner_id": 101}));
        g.add_step(3, "unrelated", &json!({"x": "y"}), &[]);
        g.record_output(3, &json!({"z": 1}));
        g.add_step(4, "insert_order", &json!({"user_id": 101}), &[explicit(2, "owner_id", "user_id")]);
    ...
    fn truncate_removes_later_steps_and_edges() {
        let mut g = DependencyGraph::new();
        for step in 1..=4 {
            let input = if step == 1 { json!({}) } else { json!({"v": step - 1}) };
