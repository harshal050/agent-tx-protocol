...
[Documentation](docs/introduction.md) · [Quickstart](docs/quickstart.md) · [Benchmarks](#benchmarks) · [Contributing](CONTRIBUTING.md)
...
The same scripted agent and the same tools, run through a naive retry-and-restart loop (**before**) and through AgentTx (**after**). Measured on an AMD Ryzen 5 5600H (12 threads), release build. Full results: [`benchmarks/results/latest.json`](benchmarks/results/latest.json) · [methodology](docs/benchmark-methodology.md).
...
Prerequisites: Rust 1.88+, a C++ toolchain and libclang (RocksDB builds from source). See [Installation](docs/installation.md).
...
Then follow the [Quickstart](docs/quickstart.md) to trigger your first dependency-jump rollback with `grpcurl`, or wire up your agent with the [client integration guide](docs/client-integration.md) (Python, TypeScript, Rust).
...
Read [Guarantees & limits](docs/guarantees.md) for exactly what is and isn't guaranteed.
...
The website reads `docs/` and `benchmarks/results/` from GitHub in production, falling back to the local checkout. A push webhook refreshes it. See [`apps/docs/README.md`](apps/docs/README.md) for deployment.
